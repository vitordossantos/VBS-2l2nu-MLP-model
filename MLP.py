...
'''
The following class definition is the custom significance class I use to choose the best training epoch, by selecting the highest validation point, as well as an agreement of less than 1% between its values for the training and 
validation datasets. I've also used it to understand if a MLP model is already very huge and unnecessary for the task of separating VBS from background
'''
...
class globalSignificance(tf.keras.metrics.Metric):
  ...
  def update_state(self, y_true, y_pred, sample_weight=None):
    ...
    self.sigPart.assign_add(self.sigSumWeight*tf.reduce_sum(isSig)) # here I set the self.sigPart variable with the physical world number of true positive events in the current batch (I have to denormalize to calculate significance)
    self.bkgPart.assign_add(self.bkgSumWeight*tf.reduce_sum(isBkg)) # here I set the self.bkgPart variable with the physical world number of false positive events in the current batch (I have to denormalize to calculate significance)
  ...
  '''
  In the result method of the globalSignificance class I calculate the projected significance. Since I'am using only the 2018 MC it's important to scale it properly to predict how great the significance would be if I was using all the 
  run II MC 1.513274 * [sqrt(2018 total number of events) / sqrt(total number of events assessed up to the current batch)] * [self.sigPart / sqrt(self.bkgPart)]. The value 1.513274 is due to the term sqrt(137.4/60), which is nothing
  more than a scale factor to prospect how great the significancy would be if the model was applied to the entire run II.
  '''
  def result(self):
...
class CustomModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        '''
        The next condition is designed to break the endless training loop in the main part of the code (which is defined outside this class, in the main code) whenever we reach the final training epoch and obtain a valid model.
        There may be times when no model is obtained due to overly demanding conditions.
        '''
	...
	''' 
        ...
        '''
        if (self.mode == 'min' and (current_val_metric < self.best_val_metric)) or (self.mode == 'max' and (current_val_metric > self.best_val_metric)): # This conditional is true whenever the model reaches another high in validation
            if train_metric is not None:  # If the training metric value is None there will be no computation!
                if train_metric != np.nan: # This conditional is important to avoid numerical calculation errors.
                  '''
                  The next line is important to compute the percentual deviation between the training and validation metric performances: "percent_diff" of the model must be lower than a predefined maximum for the model to be chosen.
                  '''
                ...
                '''
                The next conditional is in charge of registering a model that sets a new high in the validation "globalSig" metric. But will only be accepted if the "percent_diff is smaller than "self.trainValidation_percent_threshold.
                In other words: no matter if we get a new high for the validation dataset. If the train dataset deviates too much from its performance, percentually, then the model will not be chosen. If the two mentioned conditions               
                occur simultaneously, then a new best-performing model will be chosen and stored. It's noteworthy the relevance of the "self.maximum_acceptable_loss" parameter, even though I've always been setting it as "np.inf" and
                , due to this, it has had no relevance or outcome in the model choosing process.
                '''
...
while (not stopTraining):
    ...
    '''
    The next line instantiates a CustomModelCheckpoint object to be passed to the model.fit method. As one can see in the next line, we set the preset requirements such as "minimum_acceptable_val_metric", which is the minimum acceptable
    validation metric that a model must have to be chosen, the "trainValidation_percent_threshold" of 1%, which is the largest percentual deviation the validation metric value must have w.r.t the training metric value, for a model to be
    chosen, the monitor is the "globalSig" custom metric and the mode is max, pointing out that we are searchig for the highest possible metric values. The number of training epochs is also important internally for the class, the name
    of the custom metric class object and "filepath" are also important
    '''
    ...
    '''
    This next line sets the batch size, which I've prefired to set as a function of the number of batches that a dataset provides us. I've always procceed to prime factorize the "len(dataset_train)" quantity and try to set the
    number of batches as the integer divisor of "len(dataset_train)" that lies as close as possible to 1000. This is due to the appearance of negative loss values, in some training epochs, when I use a very high number of batches. The
    other issue that prevents me from using a batch size much larger than 1000 is the possibility of having bins with a negative number of events in the final histogram of the scores: this effect is due to the presence of events with
    negative weights in the 2018 MC. At the same time if I use a number of batches much smaller than 1000 I suffer from the problem of impairing the generalization capabilities of the model as well as taking the risk of converging to
    a local minimum in the optimization landscape.
    '''
